...
function animate(){
 ...
 // Check if the mixer is defined and update it
 if (window.mixer) {
    let delta = clock.getDelta() * d;  // Scale delta time with gsap's deltaRatio
...
function buildTrooper(){
    let loader = new GLTFLoader();
    loader.load(
        '/examples/models/gltf/Soldier.glb',  // Replace with the actual path to your GLB file
        function (gltf) {
            // Add the loaded GLB model to the scene
            trooper = gltf.scene;
            scene.add(trooper);
            ...
            // Animation
            const mixer = new THREE.AnimationMixer(trooper);

            // Check if the model has animations
            const animations = gltf.animations; // Use gltf.animations instead of trooper.animations
